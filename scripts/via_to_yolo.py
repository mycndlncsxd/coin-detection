...
def random_resize_rotate(coin, bg_width, bg_height):
    """
    Случайный размер (MIN_COIN_SIZE_PERCENT-MAX_COIN_SIZE_PERCENT% от фона) и поворот
    Возвращает трансформированную монету и её реальные размеры
    """
    # Используем минимальную сторону фона для определения размера монеты
    ...
    # Сохраняем пропорции монеты
    ratio = target_size / max(coin.size)
    new_w = int(coin.width * ratio)
    new_h = int(coin.height * ratio)
    
    # Убеждаемся, что монета не больше фона
    new_w = min(new_w, bg_width)
    new_h = min(new_h, bg_height)
...
def random_position(coin, bg_width, bg_height, existing_boxes, min_distance=MIN_DISTANCE):
    """
    ...
    # Если монета больше фона - центрируем её
    if coin_w > bg_width or coin_h > bg_height:
        x = max(0, (bg_width - coin_w) // 2)
        y = max(0, (bg_height - coin_h) // 2)
        return x, y
    
    for attempt in range(max_attempts):
        x = random.randint(0, bg_width - coin_w)
        y = random.randint(0, bg_height - coin_h)
        
        # Проверяем перекрытие с существующими монетами
        overlap = False
        for (ex, ey, ew, eh) in existing_boxes:
            # Проверяем с запасом min_distance пикселей
    ...
    # Если не нашли позицию - размещаем в углу
    print(f"   ⚠️ Не удалось найти позицию без перекрытия после {max_attempts} попыток")
    return 0, 0
...
def generate_synthetic_dataset(backgrounds, coins, output_dir, num_images):
    ...
    for img_idx in range(num_images):
        ...
        annotations = []
        boxes = []  # хранит (x, y, width, height) в пикселях
        ...
        for coin_num in range(num_coins):
            ...
            # 5. Изменяем размер и поворачиваем монету
            coin = random_resize_rotate(coin_orig, bg_width, bg_height)
            coin_w, coin_h = coin.size
            
            # Дополнительная проверка: если монета всё ещё больше фона
            if coin_w > bg_width or coin_h > bg_height:
                scale = min(bg_width / coin_w, bg_height / coin_h) * 0.9
                new_w = int(coin_w * scale)
                new_h = int(coin_h * scale)
                coin = coin.resize((new_w, new_h), Image.Resampling.LANCZOS)
                coin_w, coin_h = coin.size
            
            # 6. Ищем позицию для монеты
            x, y = random_position(coin, bg_width, bg_height, boxes)
            boxes.append((x, y, coin_w, coin_h))
            
            # 7. Накладываем монету на фон
            bg.paste(coin, (x, y), coin)
            
            # 8. Создаем YOLO аннотацию с НОРМАЛИЗОВАННЫМИ координатами
            ...
            # Из-за округлений могут быть незначительные выходы за пределы
            x_center = max(0.0, min(1.0, x_center))
            y_center = max(0.0, min(1.0, y_center))
            width_norm = max(0.0, min(1.0, width_norm))
            height_norm = max(0.0, min(1.0, height_norm))
            
            # Формируем строку аннотации: class x_center y_center width height
